...
点击虚线框 → 接受识别结果，区域被锁定（浅蓝填充 + 虚线边框）
       ↓
如需补充：点击「手动选择」→ 悬停高亮（蓝色描边边框）→ 点击锁定
       ↓
右侧面板展开（固定显示），显示所有已选文本
       ↓
用户可在面板中编辑 / 删除某段，调整顺序
       ↓
...
### 第三层 — 云端 LLM（可选）
- 用户预先在设置页填入 API Key（AES-GCM 加密存储）
- 直接调用 Anthropic/OpenAI，插件不做中转
...
| P1 | AI 文本清洗（可选） | 去除版权声明/广告语，格式化段落，确认后直接替换原文本 |
| P1 | API Key 设置页 | 本地 AES-GCM 加密存储，从不上传 |
